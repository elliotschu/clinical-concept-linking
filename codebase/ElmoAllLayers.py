...
class ElmoAllLayers(torch.nn.Module):
    """
    ...
    def __init__(self,
                 ...
                 learn_mix_parameters: bool = False,
                 module: torch.nn.Module = None) -> None:
        super(ElmoAllLayers, self).__init__()
        ...
        self.learn_mix_parameters = False
        if learn_mix_parameters:
            self.learn_mix_parameters = True
    ...
    def forward(self,    # pylint: disable=arguments-differ
        ...
        if self.learn_mix_parameters:
            ...
            for i in range(len(layer_activations)):
                ...
                #representation_with_bos_eos = scalar_mix(layer_activations, mask_with_bos_eos)
                if self._keep_sentence_boundaries:
                    processed_representation = layer_activations[i]
                    processed_mask = mask_with_bos_eos
                else:
                    representation_without_bos_eos, mask_without_bos_eos = remove_sentence_boundaries(
                        layer_activations[i], mask_with_bos_eos)
                    processed_representation = representation_without_bos_eos
                    processed_mask = mask_without_bos_eos
                representations.append(self._dropout(processed_representation))
        ...
        """
        return {'elmo_representations': representations, 'mask': processed_mask}
